...
def build_mac(enable_xnnpack = False, debug = False):
    run_cmd(f'bazel build --config=macos {get_options(enable_xnnpack, debug)} tensorflow/lite/c:tensorflowlite_c')
    copy('bazel-bin/tensorflow/lite/c/libtensorflowlite_c.dylib', get_dst_path('darwin', debug))
    ...
    copy('bazel-bin/tensorflow/lite/delegates/gpu/tensorflow_lite_gpu_dylib.dylib', get_dst_path('darwin', debug))
    # Restore it
    patch(cpuinfo_file, patched, original)

def build_windows(enable_xnnpack = False, debug = False):
    run_cmd(f'bazel build {get_options(enable_xnnpack, debug)} tensorflow/lite/c:tensorflowlite_c')
    copy('bazel-bin/tensorflow/lite/c/tensorflowlite_c.dll', get_dst_path('windows', debug))
    if debug:
        copy('bazel-bin/tensorflow/lite/c/tensorflowlite_c.pdb', get_dst_path('windows', debug))


def build_linux(debug = False):
    run_cmd(f'bazel build {get_options(False, debug)} --cxxopt=--std=c++11 tensorflow/lite/c:tensorflowlite_c')
    copy('bazel-bin/tensorflow/lite/c/libtensorflowlite_c.so', get_dst_path('linux', debug))

def build_ios(debug = False):
    run_cmd(f'bazel build --config=ios_fat {get_options(False, debug)} //tensorflow/lite/ios:TensorFlowLiteC_framework')
    unzip('bazel-bin/tensorflow/lite/ios/TensorFlowLiteC_framework.zip', 'iOS')
    # Metal Delegate
    run_cmd(f'bazel build {get_options(False, debug)} --config=ios_fat //tensorflow/lite/ios:TensorFlowLiteCMetal_framework')
...
def build_android(enable_xnnpack = False, debug = False):
    run_cmd(f'bazel build --config=android_arm64 {get_options(False, debug)} //tensorflow/lite/c:libtensorflowlite_c.so')
    copy('bazel-bin/tensorflow/lite/c/libtensorflowlite_c.so', get_dst_path('armv8-a', debug))
    run_cmd(f'bazel build --config=android_arm {get_options(False, debug)} //tensorflow/lite/c:libtensorflowlite_c.so')
    copy('bazel-bin/tensorflow/lite/c/libtensorflowlite_c.so', get_dst_path('armeabi-v7a', debug))
