  ...
  for (int i = 0; i < schedulers.size(); ++i) {
    if (schedulers[i] == kFixedDevice) {
      ...
      schedulers_.emplace_back(
          new HeterogeneousEarliestFinishTimeScheduler(this));
...
bool Planner::IsSLOViolated(Job& job) {
  ...
  if (job.slo_us > 0) {
    ...
    int64_t expected_latency =
        workers_waiting_[job.worker_id] + job.expected_execution_time;
  ...
  if (log_file.is_open()) {
    while (!jobs_finished_.queue.empty()) {
      ...
      log_file << job.sched_id << "\t" << job.job_id << "\t" << job.model_fname
               ...
               << job.worker_id << "\t" << job.subgraph_idx << "\t"
...
void Planner::UpdateJobScheduleStatus(Job& job, Subgraph* target_subgraph) {
  ...
  job.expected_execution_time =
      interpreter_->GetExpectedLatency(job.subgraph_idx);
