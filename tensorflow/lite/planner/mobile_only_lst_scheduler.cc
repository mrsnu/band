...
void MobileOnlyLstScheduler::Schedule(JobQueue& requests) {
...
int64_t MobileOnlyLstScheduler::GetSlackTime(int64_t current_time,
                                               const Job& job) {
  if (job.slo_us > 0) {
...
void MobileOnlyLstScheduler::SortBySlackTime(JobQueue& requests,
                                               int window_size,
...
void MobileOnlyLstScheduler::UpdateExpectedLatency(JobQueue& requests,
                                                     int window_size) {
  for (auto it = requests.begin(); it != requests.begin() + window_size; ++it) {
    it->expected_latency =
        GetInterpreter()
