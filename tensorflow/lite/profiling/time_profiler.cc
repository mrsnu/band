...
uint32_t TimeProfiler::BeginEvent(const char* tag, EventType event_type,
  ...
  if(tag == "invoke_subgraph") {
    ...
    // Returns current timeline's index as an event handle
    return invoke_timeline_vector_.size() - 1;
  } else
    return 0;
}

void TimeProfiler::EndEvent(uint32_t event_handle) {
  if(event_handle < invoke_timeline_vector_.size()) {
    invoke_timeline_vector_[event_handle].second = std::chrono::system_clock::now();
  }
}
