...
TfLiteStatus Subgraph::SetNextSubgraph(Subgraph* next) {
  std::set<int> potential_output_tensors = outputs();
  if (
}
...
Subgraph* Subgraph::GetNextSubgraphs() const {
  return next_;
}

Subgraph* Subgraph::GetPrevSubgraphs() const {
  return prev_;
}
