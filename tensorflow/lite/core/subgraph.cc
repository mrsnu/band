...
TfLiteStatus Subgraph::Invoke() {
  ...
  if (!delegates_applied_.empty())
    delegate_name = TfLiteDelegateGetName(
      static_cast<TfLiteDelegateFlags>(delegates_applied_.front()->flags));
  TFLITE_SCOPED_SUBGRAPH_PROFILE(profiler_.get(), delegate_name, model_index_);
  if (!consistent_) {
