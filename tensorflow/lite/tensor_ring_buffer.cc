...
TensorRingBuffer::TensorRingBuffer(ErrorReporter* error_reporter,
  ...
  for (size_t i = 0; i < size_; i++) {
    ...
    for (size_t j = 0; j < tensors_[i].size(); j++) {
      TfLiteTensorReset(tensors[j]->type, tensors[j]->name, tensors[j]->dims,
...
size_t TensorRingBuffer::Alloc() { return head_++; }

bool TensorRingBuffer::IsValid(size_t handle) const {
  if ((head_ / size_) > (handle / size_) && GetIndex(head_) >= handle) {
    return false;
  } else {
    return true;
  }
...
const std::vector<TfLiteTensor>* TensorRingBuffer::Get(size_t handle) const {
  return IsValid(handle) ? &tensors_[GetIndex(handle)] : nullptr;
}

TfLiteStatus TensorRingBuffer::Put(std::vector<TfLiteTensor*> tensors,
                                   size_t handle) {
  if (!IsValid(handle)) {
    TF_LITE_REPORT_ERROR(error_reporter_, "Invalid memory handle: %d", handle);
    return kTfLiteError;
  }
...
size_t TensorRingBuffer::GetIndex(size_t handle) const {
  return handle % size_;
}
}  // namespace tflite