...
def get_bazel_options(
    ...
    opt += f"--config {PLATFORM[platform]}" + ("" if backend == "none" else f" --config {backend}") + ("" if trace == False else " --config trace") + ("" if opencl == False else " --define gpu=nvidia-opencl --copt=-DCL_TARGET_OPENCL_VERSION=200 --copt=-DCL_DELEGATE_NO_GL --copt=-DEGL_NO_X11") + " "
    return opt
...
def get_argument_parser(desc: str):
    ...
    parser.add_argument('-opencl', action="store_true", default=False,
                        help='Test on Linux Nvidia GPU with OpenCL support')
    return parser
