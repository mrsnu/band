...
## Notes on Offloading
All server-related files are put in `server/`.
Within a Python virtual environment (virtualenv or conda, etc.), install gRPC with ([link](https://grpc.io/docs/languages/python/quickstart/)):
```
pip install grpcio grpcio-tools
```

The pb2 py files were generated by running the following command in `server/`:
```
python -m grpc_tools.protoc -I../tensorflow/lite/proto --python_out=. --grpc_python_out=. ../tensorflow/lite/proto/helloworld.proto
```
...
The 7-th scheduler is `OffloadingScheduler`, which blindly offloads all job to the server.
The server's address is given by "offloading_target".
This address must equal the server's address, and the port specified in server.py
```
{
    "models": [
        {
            "graph": "/data/local/tmp/models/mobilenet_v1_1.0_224_quant.tflite",
            "period_ms": 10,
            "batch_size": 1,
            "input_layer" : "input",
            "input_layer_shape" : "1,224,224,3",
    ...
    "availability_check_interval_ms": 30000,
    "schedule_window_size": 10,
    "offloading_target": "11.elsa.snuspl.snu.ac.kr:50051"
}
```
