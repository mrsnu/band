...
Json::Value LoadFromFile(std::string file_path) {
  if (!FileExists(file_path)) {
    BAND_LOG(LogSeverity::kWarning, "There is no such file %s",
                  file_path.c_str());
    return {};
  }
  ...
  if (IsEmpty(in)) {
    BAND_LOG(LogSeverity::kWarning, "File %s is empty", file_path.c_str());
    return {};
  }
...
bool Validate(const Json::Value& root, std::vector<std::string> required) {
  if (root.isNull()) {
    BAND_LOG(LogSeverity::kError, "Please validate the json config file");
    return false;
  }
