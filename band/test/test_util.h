...
struct MockContextBase : public Context {
  ...
  using ShortestLatencyWithUnitSubgraph = std::pair<std::vector<SubgraphKey>, int64_t>;
  using SubgraphWithShortestLatency = std::pair<std::vector<SubgraphKey>, int64_t>;
  MOCK_CONST_METHOD5(GetShortestLatency, 
                     std::pair<SubgraphKey, int64_t>(int, std::set<int>, int64_t, WorkerWaiting, SubgraphKey));

  MOCK_CONST_METHOD3(GetShortestLatencyWithUnitSubgraph, 
                     ShortestLatencyWithUnitSubgraph(int, int, WorkerWaiting));
  MOCK_CONST_METHOD2(GetSubgraphWithShortestLatency, SubgraphWithShortestLatency(Job&, WorkerWaiting));
  MOCK_CONST_METHOD3(GetSubgraphIdxSatisfyingSLO, SubgraphKey(Job&, WorkerWaiting, const std::set<WorkerId>&));
