...
namespace Band {
HEFTReservedScheduler::HEFTReservedScheduler(int window_size)
    : window_size_(window_size) {}

ScheduleAction HEFTReservedScheduler::Schedule(const Context& context,
                                               JobQueue& requests) {
  ...
  while (window_size > 0) {
      ...
      for (auto it = requests.begin(); it != requests.begin() + window_size;
        ...
        for (auto job_subgraph_key : reserved_) {
          ...
          reserved_time[job_subgraph_key.second.GetWorkerId()] +=
              context.GetExpected(job_subgraph_key.second);
        }

        std::pair<std::vector<SubgraphKey>, int64_t> best_subgraph =
            context.GetSubgraphWithShortestLatency(job, reserved_time);
    ...
    action[target_subgraph_key.GetWorkerId()].push_back({job, target_subgraph_key});
